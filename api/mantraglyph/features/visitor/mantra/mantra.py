...
from ....core.shared.utils.variables import WURMIC_BRAVO, MANTRA_PATH
...
class CreateMantraFeature(graphene.Mutation):
    ...
    def mutate(cls, root, info, **kwargs):
        ...
        mantra = scant_tools_service.applyTranslations(
            image, sentence_to_word, coord, WURMIC_BRAVO, text_color
        )
        logging.warning(MANTRA_PATH)
        mantra.save(MANTRA_PATH)
        mantra = image_service.save_image_in_firebase(MANTRA_PATH)
        mantraglyph = models.Mantraglyph.objects.create(mantra=mantra)

        logging.warning("ici16")
        return CreateMantraFeature(mantraglyph=mantraglyph, success=True)
